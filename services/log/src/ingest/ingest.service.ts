...
export class IngestService {
  ...
  async ingest(taskId: string, entry: LogEntryRequest): Promise<{ acknowledged: true; logCount: number }> {
    ...
    this.logCount++;
    this.logger.debug(`Log ingested: ${taskId} [${entry.level}] ${entry.step} — ${entry.message.slice(0, 80)}`);
