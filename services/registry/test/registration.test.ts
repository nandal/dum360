...
describe('Node Registration Flow', () => {
  ...
  it('should return JWT and nodeId on successful registration', () => {
    ...
    const expectedFields = ['nodeId', 'jwt', 'heartbeatInterval', 'pollInterval', 'attestedCapabilities', 'failedAttestations'];
  ...
  it('should return attested and failed capabilities separately', () => {
    ...
    const response = {
      attestedCapabilities: { executors: [{ id: 'github' }], resources: [], tools: [], runtimes: [], services: [] },
  ...
  it('should reject registration with no executors declared', () => {
    ...
    const capabilities = { executors: [], resources: [], tools: [], runtimes: [], services: [] };
