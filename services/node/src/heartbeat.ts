...
export class HeartbeatLoop {
  ...
  start(): void {
    ...
    this.timer = setInterval(() => this.sendHeartbeat(), this.config.heartbeatInterval * 1000);
  }
  ...
  private async sendHeartbeat(): Promise<void> {
    try {
      ...
      await this.client.heartbeat({
        status: 'online',
        resources: {
          cpu: { used: Math.round((process.cpuUsage().user / 1e6) * 100) / 100, total: cpuCount },
          ram: { used: `${Math.round(usedRam / 1e9)}GB`, total: `${Math.round(totalRam / 1e9)}GB` },
        },
