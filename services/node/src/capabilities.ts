...
type ProbeMap = { tool: string; category: 'tool' | 'runtime' | 'service' | 'executor' };
...
async function probeBinary(name: string): Promise<{ found: boolean; version?: string }> {
  try {
...
export async function detectCapabilities(): Promise<DeclaredCapabilities> {
  ...
  for (const { tool, category } of PROBES) {
    ...
    if (found) {
      ...
      switch (category) {
        case 'executor':  result.executors.push(cap); break;
        case 'tool':       result.tools.push(cap); break;
        case 'runtime':    result.runtimes.push(cap); break;
        case 'service':    result.services.push(cap); break;
      }
    }
  }
