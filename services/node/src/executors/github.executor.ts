...
export class GitHubExecutor implements Executor {
  ...
  async execute(
    ...
    const log = (step: string, message: string, level: LogEntry['level'] = 'info') =>
    ...
    try {
      ...
      await this.run(
        `${this.aiCli} --prompt-file ${promptFile} --output-dir ${workDir}`,
        workDir,
      );
      ...
      await this.run(
        `git commit -m "dum360: ${task.issue?.title ?? task.instructions.slice(0, 72)}"`,
        workDir,
      );
      ...
      const prResult = await this.run(
        `gh pr create --title "${task.issue?.title ?? 'DUM360: AI Fix'}" --body "🤖 Automated PR by DUM360\n\nCloses #${task.issue?.number ?? '?'}" --base ${task.branch}`,
        workDir,
      );
      ...
      return {
        branch,
        prUrl,
        diff: await this.run('git diff --stat', workDir).then((r) => r.stdout.trim()),
      };
    } catch (error) {
  ...
  async probe(): Promise<{ passed: boolean; reason?: string }> {
    try {
      execSync(`${this.gitPath} --version`, { stdio: 'pipe' });
      execSync(`${this.ghPath} --version`, { stdio: 'pipe' });
      return { passed: true };
    } catch {
