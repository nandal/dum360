...
async function main(): Promise<void> {
	...
	console.log(
		`[register] Registering as "${config.nodeName}" with ${config.serverUrl}...`,
	);
	...
	console.log(
		`[register] Attested: ${JSON.stringify(
			Object.values(registration.attestedCapabilities)
				.flat()
	...
	if (registration.failedAttestations.length > 0) {
		console.warn(
			`[register] Failed attestations: ${JSON.stringify(registration.failedAttestations)}`,
		);
	}
	...
	for (const executor of executors) {
		...
		console.log(
			`[executor] ${executor.id()}: ${probe.passed ? "ready" : "not available — " + probe.reason}`,
		);
	}
