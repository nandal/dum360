...
function parseCmd(body: string): string | null {
  if (!body) return null;
  const i = body.indexOf(TRIGGER); if (i === -1) return null;
...
interface Node { id: string; status: string; caps: string[]; cpuU: number; cpuT: number; }
function sched(taskCaps: string[], nodes: Node[]): Node | null {
  const online = nodes.filter(n => n.status === 'online' && n.cpuU < n.cpuT);
  const capable = online.filter(n => taskCaps.every(c => n.caps.includes(c)));
...
describe('Scheduler Algorithm', () => {
  const ns: Node[] = [
    { id:'n1',status:'online',caps:['git','claude','gh','docker'],cpuU:8,cpuT:16},
    { id:'n2',status:'online',caps:['git','claude','gh'],cpuU:2,cpuT:8},
    { id:'n3',status:'busy',caps:['git','claude','gh','docker'],cpuU:12,cpuT:16},
