...
describe('Task Lifecycle Flow (Integration)', () => {
  it('FLOW: Create task → status=queued → assign → status=running → complete → status=completed', async () => {
     ...
     */
    const flow = [
      { step: 'create', method: 'POST', path: '/tasks', expected: 201, status: 'queued' },
      { step: 'assign', method: 'GET', path: '/tasks/next', expected: 200, status: 'running' },
      { step: 'complete', method: 'PATCH', path: '/tasks/TID/result', expected: 200, status: 'completed' },
      { step: 'verify', method: 'GET', path: '/tasks/TID', expected: 200, status: 'completed' },
    ];
  ...
  it('FLOW: Task fails → retry → queued → assign → complete', async () => {
     ...
     */
    const flow = ['queued', 'running', 'failed', 'queued', 'running', 'completed'];
  ...
  it('FLOW: Task detail includes requirements and state transitions', async () => {
     ...
     */
    const taskDetail = {
      requirements: [{ capabilityName: 'git' }, { capabilityName: 'claude' }, { capabilityName: 'gh' }],
      stateTransitions: [
...
describe('Concurrent Nodes (Integration)', () => {
  ...
  it('FLOW: Node fails mid-task → task re-queued → new node picks up', async () => {
     ...
     */
    const recoveryPath = ['node-1_executing', 'node-1_offline', 'task_failed', 'task_requeued', 'node-2_assigned', 'completed'];
...
describe('Error Response Format (Integration)', () => {
  ...
  it('400 validation errors include { messages: [{ field, message }] }', () => {
    const expected = { statusCode: 400, error: 'Validation Failed', messages: [{ field: 'x', message: 'y' }] };
