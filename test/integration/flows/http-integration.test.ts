...
describe('Database — Schema Isolation', () => {
  it('registry schema owned by Registry Service only', () => {
    const registryTables = ['nodes', 'capabilities', 'node_capabilities', 'attestation_results', 'heartbeats'];
    expect(registryTables).toHaveLength(5);
  });

  it('orchestration schema owned by Orchestration Service only', () => {
    const orchestrationTables = ['tasks', 'task_requirements', 'task_state_transitions', 'webhook_events', 'scheduler_assignments'];
